...
const useDropzone = <TUploadRes, TUploadError = string>(
  ...
  const _uploadFileRef = useRef<(file: File, id: string, tries?: number) => Promise<void>>(async () => { });
    ...
    async (file: File, id: string, tries = 0) => {
      ...
      if (result.status === "error") {
        if (autoRetry === true && tries < (maxRetryCount ?? Infinity)) {
          ...
          return _uploadFileRef.current(file, id, tries + 1);
        }
  ...
  useEffect(() => {
    _uploadFileRef.current = _uploadFile;
  }, [_uploadFile]);
